...
s_int8 epoll_start_timer(epoll_timer_cb_t timer_cb,u_int8 sig, u_int32 time, u_int8 reload)
{
	eTimer_t 		*ptimer;
	s_int8			ret = ES_FAILED;
	epoll_log(DBG_ETIMER,"epoll_start_timer:cb:%08x,sig:%d,time:%d,isreload:%d\r\n",(int)timer_cb,sig,time,reload);
	_timer_init();
	epoll_enter_critical();
	ptimer = _find_timer(pTimerList,(void*)timer_cb,sig);
	if(ptimer!=NULL)
	{
		epoll_log(DBG_ETIMER,"epoll_start_timer:find old and update\r\n");
		_update_timer(ptimer,time,reload);
	}
	else
	{
		epoll_log(DBG_ETIMER,"epoll_start_timer: create timer\r\n");
...
s_int8 epoll_start_timer_task(epoll_task_t task,u_int32 event, u_int32 time, u_int8 reload)
{
	...
	u_int8			sig;
	s_int8			ret = ES_FAILED;
	epoll_log(DBG_ETIMER,"epoll_start_timer_task:cb:%08x,sig:%d,time:%d,isreload:%d\r\n",(int)task,event,time,reload);
	_timer_init();
...
s_int8 epoll_stop_timer(epoll_timer_cb_t timer_cb,u_int8 signal)
{
	...
	if(ptimer!=NULL)
	{
		ptimer->state = TIMER_STATE_INVALID;//delete on next time to poll at epoll_core.c
	}
	epoll_exit_critical();
	return ret;
}
...
void epoll_timer_clean(void)
{
	...
	while(1)
	{
		...
		if(ptimer!=NULL)
		{
			epoll_log(DBG_ETIMER,"epoll_timer_clean: cb:%x, sig:%d \r\n",(int)ptimer->cb.timer_cb,ptimer->sig);
			epoll_list_delete((void**)&pTimerList,ptimer);
		}
		else
		{
			break;
...
void epoll_update_tick(u_int32 tick)
{
	...
	if(ms>=1000)
	{
		second++;
		ms -= 1000;
		epoll_log(DBG_ETIMER,"epoll_update_timer: mssec:%d, sec:%d\r\n",mssecond,second);
	}
	_updata_tick(tick);
}
...
{
	u_int32 temp;
	temp = epoll_sub32_loop(mssecond,pm->start);
	if(temp>=pm->timeout)
	{
		return ES_TRUE;
	}
...
static void _timer_init(void)
{
	...
	{
		epoll_create_task(_timer_task,255);
		_init_flag = ES_TRUE;
	}
}
#include "stdint.h"
		...
		if(pcur_timer->state!=TIMER_STATE_INVALID)
		{
			...
			if(pcur_timer->runtime==0)
			{
				...
				if(pcur_timer->state==TIMER_STATE_RELOAD_TIMER)
				{
					...
					epoll_log(DBG_ETIMER,"is reload timer: cb:%x sig:%d \r\n",(int)pcur_timer->cb.timer_cb,pcur_timer->sig);
				}
				else
				{
					pcur_timer->state = TIMER_STATE_INVALID;
					epoll_log(DBG_ETIMER,"is normal timer and free: cb:%x sig:%d \r\n",(int)pcur_timer->cb.timer_cb,pcur_timer->sig);
				}
				if(pcur_timer->ttype==TIMER_TPYE_FORTASK)
				{				
					epoll_log(DBG_ETIMER,"time for task: cb:%x, runtime:%d, timeout:%d, sig:%d \r\n",(int)pcur_timer->cb.timer_cb, pcur_timer->runtime,pcur_timer->timeout,pcur_timer->sig);
					event = _convert_to_event(pcur_timer->sig);
					epoll_unsafe_set_event(pcur_timer->cb.task,event);
				}
				else
				{
					epoll_log(DBG_ETIMER,"time for timer: cb:%x, runtime:%d, timeout:%d, sig:%d \r\n",(int)pcur_timer->cb.timer_cb, pcur_timer->runtime,pcur_timer->timeout,pcur_timer->sig);
					epoll_unsafe_set_event(_timer_task,1);
				}
			}
		}
		else
...
static void _timer_task(u_int32 evt, void *arg0, u_int32 len)
{
	epoll_log(DBG_ETIMER,"_timer_task! evt:%x \r\n",evt);
	while(_timer_process()==ES_TRUE);
}
static s_int8 _timer_process(void)
{
	eTimer_t 			*pcur_timer;
	epoll_timer_cb_t	cb = NULL;
	...
	while(pcur_timer!=NULL)
	{
		if(pcur_timer->expire==ES_TRUE)
		{
			if(pcur_timer->ttype!=TIMER_TPYE_FORTASK)
			{
				...
				busy_flag = ES_TRUE;
				epoll_log(DBG_ETIMER,"_timer_process: cb:%x, runtime:%d, timeout:%d, sig:%d \r\n",(int)cb, pcur_timer->runtime,pcur_timer->timeout,pcur_timer->sig);
				break;
			}
			else
			{
				epoll_log(DBG_ETIMER,"_timer_process: cb:%x not for timer \r\n",(int)cb);
			}
		}
		if(pcur_timer->state==TIMER_STATE_INVALID)
		{
...
static eTimer_t *_update_timer(eTimer_t *ptimer, u_int32 time, u_int8 reload)
{
	epoll_log(DBG_ETIMER,"_update_timer !\r\n");
	ptimer->runtime = time;
...
{
	eTimer_t *ptimer;
	epoll_log(DBG_ETIMER,"_create_timer !\r\n");
	ptimer = epoll_malloc(sizeof(eTimer_t));
	if(ptimer!=NULL)
	{
		epoll_memset(ptimer,0,sizeof(eTimer_t));
		_update_timer(ptimer,time,reload);
		if(ttype == TIMER_TPYE_FORTIMER)
			ptimer->cb.timer_cb = (epoll_timer_cb_t)timer_cb;
		else if(ttype == TIMER_TPYE_FORTASK)
			ptimer->cb.task = (epoll_task_t)timer_cb;
		ptimer->sig = sig;
		ptimer->ttype = ttype;
		epoll_list_insert((void**)&pTimerList,ptimer);
	}
	return ptimer;
}
static s_int32 _get_next_timeout(eTimer_t *plist)
...
static u_int8 _convert_to_sig(u_int32 event)
{
	...
	epoll_log(0,"_convert_to_sig:sig:%d\r\n",i);
	return i;
}
static u_int32 _convert_to_event(u_int8 sig)
{
	u_int32 evt = (1<<sig);
	epoll_log(0,"_convert_to_sig:evt:%08x\r\n",evt);
	return (1<<sig);
}
