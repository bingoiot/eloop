...
{
	char *str;
	str = (char*)epoll_malloc(len+1);
	if(str!=NULL)
	{
		epoll_memcpy(str,pdata,len);
		str[len] = '\0';
	}
	return str;
}
...
s_int32 epoll_ftoa(char *str, float num, s_int32 n)        //n��ת���ľ��ȣ������ַ���'.'���м�λС��
{
    s_int32     sumI;
    float   sumF;
    s_int32     sign = 0;
...
char  *epoll_case_strstr(char *str0,char *str1)
{
	...
	for(i=0;str0[i]!='\0';)
	{
		start = i;
		j = 0;
		while(epoll_toupper(str0[i])==epoll_toupper(str1[j]))
		{
			if(str1[j+1]=='\0')
				return &str0[start];
			if((str0[i]=='\0')||(str0[i]==')')||(str0[i]=='('))
