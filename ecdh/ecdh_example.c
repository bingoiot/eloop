...
void vprintb(char *s, unsigned char *pdata, int len)
{
  int i;
  printf("%s",s);
  for(i=0;i<len;i++)
...
static void ecdh_demo(void)
{
  ...
  if (!initialized)
  {
    prng_init(4);//(0xbad ^ 0xc0ffee ^ 42) | 0xcafebabe | 666);
    initialized = 1;
  }
  ...
  vprintb("\r\n ecdh_demo, public A key:",puba,sizeof(puba));
  vprintb("\r\n ecdh_demo, private A key:",prva,sizeof(prva));
  /* 2. Bob picks a (secret) random natural number 'b', calculates Q = b * g and sends Q to Alice. */
  for (i = 0; i < ECC_PRV_KEY_SIZE; ++i)
  {
    prvb[i] = prng_next();
  ...
  vprintb("\r\n ecdh_demo, public B key:",pubb,sizeof(pubb));
  vprintb("\r\n ecdh_demo, private B key:",prvb,sizeof(prvb));
  /* 3. Alice calculates S = a * Q = a * (b * g). */
  assert(ecdh_shared_secret(prva, pubb, seca));
  vprintb("\r\n ecdh_demo, private A securite key:",seca,sizeof(seca));
  /* 4. Bob calculates T = b * P = b * (a * g). */
  assert(ecdh_shared_secret(prvb, puba, secb));
  vprintb("\r\n ecdh_demo, private B securite key:",secb,sizeof(secb));
...
/* int ecdsa_sign(const uint8_t* private, const uint8_t* hash, uint8_t* random_k, uint8_t* signature);
  ...
  vprintb("\r\n ecdsa_broken, public key:",pub,sizeof(pub));
  vprintb("\r\n ecdsa_broken, public key:",prv,sizeof(prv));
  /* No asserts - ECDSA functionality is broken... */
  ecdsa_sign((const uint8_t*)prv, msg, k, signature);
  int ret = ecdsa_verify((const uint8_t*)pub, msg, (const uint8_t*)signature); /* fails... */
  printf("\r\n ecdsa_broken result:%d\r\n",ret);
}
